...
Pre-W1 已建立机器可检查的 **Required Release Scope Catalog**、内容寻址 **Evidence Manifest**、只追加 Gate Registry 与连续 Registry Checkpoint 的最小控制面；进入 W1 后仍须补齐 **Case Evidence Requirement Registry**、**Case Verification Registry**、**Golden Journey & Invariant Verification Registry**，并把构建能力与真实运行绑定分别登记。最终发布判定读取下列十一类记录及候选制品元数据，不通过解析自然语言、手工删 scope、截断失败记录或改写设计 CSV 完成。

### 1.1 单一维护者治理

v0.1 采用显式的 `SOLE_MAINTAINER` 治理：唯一产品决策权威为 `github:AnnCYW-cm`，版本化决策角色为 `rolefox-sole-maintainer-v1`。同一维护者编制、提交、复核并批准 Required Release Scope Catalog、研究协议、Candidate Scope、Gate、`ACCEPTED_FALLBACK` 与 `FUTURE/N_A` 范围决定；在这一协议版本下，已决定 Gate 必须满足 `submitted_by == approved_by == "github:AnnCYW-cm"`。两个字段和对应时间仍分别保存，且批准必须绑定完整 canonical payload、全部输入 digest、稳定身份、角色版本和不可伪造的 proof，不能用裸字符串、口头确认或聊天记录放行。

人员职责不分离不等于证据与决定合并。研究标准和 cohort 必须预先冻结，Evidence 必须可复算、内容寻址并保留完整分母，Gate/Checkpoint 必须只追加，任何缺证、时间倒置、摘要漂移或信任验证失败仍然失败关闭。受保护 `main` 上的 GitHub Actions OIDC workload 只提供机器签名身份及构建/工作流来源证明，不是产品决策者，不得写入 `approved_by` 或替维护者形成 Gate 结论；checkpoint 的外部锚定仍是单独且必需的信任条件。

本规则只适用于显式绑定上述治理模型和角色版本的新记录。既有不可变 snapshot、旧 criteria version、Gate 事件和 checkpoint 仍按其创建时的独立审批语义及当时 verifier/toolchain 验证；不得原地改写、追认或把旧 pending/blocked 记录解释成已获单一维护者批准。治理迁移必须生成新的 catalog/protocol/spec/candidate 内容寻址版本，并在旧 Gate/checkpoint 链之后追加新事件。

## 2. 十一类权威记录

| 记录 | 权威固定路径 | 生命周期 | 作用 |
| --- | --- | --- | --- |
| Spec Manifest | `verification/spec-manifest-v0.1.json` 与 `verification/spec-manifests/` | Accepted 规范集合或 verifier 工具链变化时生成新快照 | 当前文件指向本轮输入；内容寻址快照以显式逐文件 authority/status 清单枚举 normative 文件、逐文件 digest、集合 digest、canonicalization、算法与 verification toolchain file-set digest；Draft/Proposed 文档不取得规范效力 |
| Required Release Scope Catalog | `verification/required-release-scopes-v0.1.json` | 从 Accepted 产品基线逐项编译、由唯一维护者逐项复核并签名接受；基线变化时创建新版本 | 以稳定 `scope_id` 闭世界登记 spec 强制能力、channel role、最低/最高模式、`required_gate_ids`、是否允许 fallback；checker 不从自然语言临时猜 scope |
...
| `evidence_manifest_refs` | `VERIFIED` 时至少一项；每项引用不可变 `(evidence_id, manifest_digest, record_digest)`：前两项固定待 attestation 的 payload，`record_digest` 固定最终 attestation/proof，三者必须互相校验 |
...
| `approved_by` / `approved_at` | `FUTURE/N_A` 时必填；单一维护者模式允许与提交作者相同，但必须分别记录提交与批准事件和时间 |
| `approver_role_version` / `approval_proof_digest` | 必须绑定 `github:AnnCYW-cm`、`rolefox-sole-maintainer-v1` 与覆盖完整 canonical payload 的可验证 proof；普通字符串、口头确认或聊天记录不能充当批准证明 |
...
每份 manifest 必须分两层内容寻址：先对不含 `evidence_id`、`manifest_digest`、`record_digest` 和 attestation/signature 字段的 canonical manifest body 取 SHA-256，令 `evidence_id=ev_<digest>` 且 `manifest_digest=<digest>`；再由受信 CI workload identity 或获准 operator 对该 payload digest、candidate/spec 和 run identity 生成 attestation，最后对包含 ID、payload digest 与完整 attestation、只排除 `record_digest` 自身的 canonical record 取 SHA-256，得到 `record_digest`。相同 `evidence_id` 的最终记录出现不同字节或 `record_digest`、缺少可验证 attestation、producer 不在当前 allowlist，或同一路径被改写时一律判为篡改并失败关闭；修复或重跑必须生成新 payload ID，Gate/Case/Journey/Invariant 引用均保存 `(evidence_id, manifest_digest, record_digest)`。

每份 manifest 至少包含：

- `evidence_id`、`manifest_digest`、最终 `record_digest`、producer/attestation 与一个或多个 typed `subject_refs`：`CASE_ASSERTION(case_id, assertion_id)`、`GOLDEN_JOURNEY(gold_id, assertion_id)`、`INVARIANT(invariant_id, assertion_id)`、`GATE_CRITERION(gate_id, criterion_id)` 或 `METRIC(metric_id)`；只有 `CASE_ASSERTION`、`GOLDEN_JOURNEY` 与 `INVARIANT` 强制提供各自稳定 ID/assertion ID，Pre-W1 研究、Gate-only 证据与跨主题指标不得伪造 Case 引用；
...
每条 Gate 记录必须包含 `record_id`、`gate_id`、结构化 scope/`scope_id`、criteria version、不可变输入 evidence `(evidence_id,manifest_digest,record_digest)`、`result`、`candidate_artifact_kind`、candidate artifact digest、spec digest、Required Release Scope Catalog digest、Candidate Scope Manifest ID/digest、verification toolchain digest、`runtime_binding_manifest_refs`（适用 manifest 的 `(id,digest)`）、`submitted_by`、`approved_by`、判定时间、`approver_role_version`、`approval_proof_digest`、同一 `gate_id+scope_id` 的 `previous_record_id`，以及需要时的 `predecessor_gate_refs` 与 `not_before`。`SPEC_OR_EXPERIMENT` Gate 记录引用同类 Scope Manifest 与非空 research/spec candidate digest，且 runtime refs 必须为空；`BUILD` Gate 记录引用同一候选制品 introspection 生成的 BUILD manifest，涉及真实数据/binding 时 `runtime_binding_manifest_refs` 必须非空并精确覆盖该 Gate 全部运行实例。对当前协议下已决定的 Gate，validator 要求 `submitted_by == approved_by == "github:AnnCYW-cm"` 且 `approver_role_version == "rolefox-sole-maintainer-v1"`；approval proof 必须覆盖整条 canonical record 及所有输入 digest，记录或 manifest 改变会使 proof 失效。GitHub Actions OIDC 只验证工作流与制品来源，不能作为 `approved_by` 或产品决定 proof 的替代品。
...
只追加不能只靠仓库内文件自我声明。每批 Gate/Evidence/Case/Journey/Invariant 记录追加后都生成新的 `Registry Integrity Checkpoint`：以连续 `sequence` 引用上一 `(checkpoint_id,root_digest,record_count)`，覆盖完整 Gate JSONL 字节 digest、排序后的全部 current-head set、Case/Journey/Invariant registry digest、全部可达 Evidence Manifest `(evidence_id,manifest_digest,record_digest)` 集、candidate/spec/catalog/research protocol/verification toolchain digest 与生成时间，并由受保护 `main` 上的 GitHub Actions OIDC workload identity 签名。OIDC 只证明机器执行和来源，不形成产品批准。每个历史 digest 必须可解析到不可变 authority/candidate 快照；genesis checkpoint 锚定受保护 CI provenance；已有发布后，新 checkpoint 还必须引用上一已发布制品 metadata/transparency entry 中的 checkpoint root。候选 release attestation 必须覆盖当前 checkpoint ID/root，且 current sequence/count/root 必须严格扩展外部锚点；删除尾部 FAIL/BLOCKED、删 evidence、回退 record count、重写旧 checkpoint，或只在同一可改写仓库中补一个新签名，都必须 `BLOCKED`。
...
历史岗位规则回放采用至少 5 位达到痛点门槛者，每位冻结 20 个岗位，并绑定逐参与者数据集 digest、硬条件覆盖全部明确拒绝原因及剩余模糊项可归入有限异常类别的结果；未达到痛点门槛的其余访谈者不得补入回放分母。Gate 1 criteria version 必须在采集前冻结 cohort、岗位集与阈值并获批准。证据包还记录选择的目标用户/通道范围、唯一维护者身份、角色版本、决定时间和 proof。未通过时 W1 状态必须为 `BLOCKED_NOT_STARTED`。
...
7. 从 Required Release Scope Catalog、Candidate Scope Manifest 与 Runtime Binding Manifest 推导的 required `(gate_id,scope_id)` 和 Gate Registry current heads 精确同集；每条链无分叉、循环或缺口。新单一维护者协议下，已决定 Gate 必须满足 `submitted_by == approved_by == "github:AnnCYW-cm"`，且 `approver_role_version` 必须是 `rolefox-sole-maintainer-v1`，审批 proof 覆盖完整 canonical record 与输入 evidence digest，别名只映射 canonical ID；旧协议记录仍执行其原有身份分离规则。当前 Registry Integrity Checkpoint 的 sequence/count/root、Gate log/full head-set、Case/Journey/Invariant 与 Evidence manifest-set digest 均与磁盘一致，签名有效，并严格扩展受保护 CI 或上一发布的外部锚点；任何尾部截断/回退均失败。Pre-W1、Gate A/B、Validation Gate 2、Gate C-pre、业务 G0/G1、G3 与 `v0.1-R` 必须 PASS；Catalog 中 `fallback_allowed=false` 的每个 Gate C-read scope（至少一个真实岗位只读源、邮箱读取、日历 busy）、业务 C-live mutation scope（邮箱回复、默认 Email notification、日历 tentative create/cancel，以及条件触发后的 update）及 heartbeat/停止告警的 `SAFETY_CONTROL_GUARD` 与 C-live scope 必须 PASS；受支持且被删除请求引用的 credential revocation 同样必须有 guard 与真实 C-live PASS，不支持时必须有 conformance、`UNSUPPORTED/NOT_ATTEMPTED` residual、官方手工入口和本地删除证据，不得伪造 C-live；`application_submit_l2` 则必须有 Gate C-live PASS，或精确满足 allowlist 的 scoped `ACCEPTED_FALLBACK`，两者不能都缺；
...
在 Pre-W1 招募、采集或回放任何 Gate 1 evidence 之前，先建立最小验证基础设施：显式逐文件状态与 verification toolchain file-set 的 Spec Manifest/digest 工具、从 Accepted spec 编译并由唯一维护者评审签名的 Required Release Scope Catalog、catalog/protocol/spec 的不可变内容寻址快照、引用这些输入且冻结 scope/cohort/criteria 的获批 `SPEC_OR_EXPERIMENT` Candidate Scope Manifest、共享 evidence enum/简写映射、两层 content-addressed Evidence Manifest schema/writer/validator，以及无分叉只追加 Gate Registry schema/writer/validator；这些工具只承载研究证据，不实现业务功能。Gate 1 的每个输入 evidence `(evidence_id,manifest_digest,record_digest)` 必须先有合法 manifest。Pre-W1 PASS 后、首个 W1 BUILD 产生前，签发绑定该 Gate head 与同一 spec/catalog/research/toolchain digest 的不可变 `w1_kickoff_attestation`；W1 启动时用当时 validator 全量回验 bootstrap records，任何不兼容、后补或时间倒置都使 Gate 1/W1 lineage 回到 `BLOCKED`。W1 的第一批工程地基再建立 BUILD Scope Manifest introspection/attestation、Runtime Binding Manifest schema/writer/validator、逐 Case Evidence Requirement Registry、Case Verification Registry、Golden Journey & Invariant Verification Registry generator 和失败关闭的完整 `release:check` 骨架；之后每个纵向切片把 Case/Golden/Invariant assertion 与满足最低 profile 的测试证据一起提交。

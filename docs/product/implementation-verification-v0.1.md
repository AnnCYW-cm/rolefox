...
只追加不能只靠仓库内文件自我声明。每批 Gate/Evidence/Case/Journey/Invariant 记录追加后都生成新的 `Registry Integrity Checkpoint`：以连续 `sequence` 引用上一 `(checkpoint_id,root_digest,record_count)`，覆盖完整 Gate JSONL 字节 digest、排序后的全部 current-head set、Case/Journey/Invariant registry digest、全部可达 Evidence Manifest `(evidence_id,manifest_digest,record_digest)` 集、candidate/spec/catalog/research protocol/verification toolchain digest 与生成时间，并由受保护 CI workload identity 签名。每个历史 digest 必须可解析到不可变 authority/candidate 快照；genesis checkpoint 锚定受保护 CI provenance；已有发布后，新 checkpoint 还必须引用上一已发布制品 metadata/transparency entry 中的 checkpoint root。候选 release attestation 必须覆盖当前 checkpoint ID/root，且 current sequence/count/root 必须严格扩展外部锚点；删除尾部 FAIL/BLOCKED、删 evidence、回退 record count、重写旧 checkpoint，或只在同一可改写仓库中补一个新签名，都必须 `BLOCKED`。

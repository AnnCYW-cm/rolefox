...
更新策略时展示策略 diff、范围、有效期、每日上限和新增风险，生成新版本并保留旧版本。恢复时只恢复关联申请。解除 `STOP_OUTBOUND`、Kill Switch 或完成备份恢复后，系统先完成对账；旧 Plan 和授权不复活，用户按 capability 恢复到 L2，健康检查与明确确认通过后才可重新进入 L3。
...
以上内容共同构成 v0.1 核心准备包；AI 生成的面试建议属于 P1，不得成为发送成功通知的依赖。默认主操作是“查看准备包”，不是“接受面试”。如果时间尚未双方确认、日历写入失败或存在冲突，应生成异常，而不是成功通知。
...
产品内 Inbox 是通知与异常的强制事实来源；邮件是默认外部通知，Webhook 是可选适配器。v0.1 不要求第二个外部备用渠道，P1 才将其设为必需。外部通知失败不得删除或改写产品内事实。
...
- 日历账户、时区、窗口和通知已验证；
- 急停、撤销和回滚入口可用。
- 对应 capability 达到门槛：任何真实外发先完成连续 7 天 Shadow；匹配/材料各 50 个决策；投递/回复各 20 次真实 L2；自动约面 5 次真实 L2 并通过 20 个合成异常 Case；未授权、重复、虚构和错误排期均为 0。
